...
import no.fun.stuff.engine.event.EventPublisher;
import no.fun.stuff.engine.event.Subscriber;
...
public class Plotter extends AbstractGame {
    ...
    private final ClickedState clickedState = new ClickedState();
...
//        camera2D = new Camera2D(screenSize, viewPort,);
        Vector2D viewPort = new Vector2D(400, 300);
...
//                                plottList.remove(size - 1);
        ...
        Subscriber saveToFile = (event, data) -> {
...
//        final String fileName = "save_#.txt";
        int mouseScroll = input.getScroll();
        if (mouseScroll != 0) {
