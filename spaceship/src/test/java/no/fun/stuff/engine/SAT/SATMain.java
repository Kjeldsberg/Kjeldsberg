...
//        }
        input.updata();
        for (int i = 1; i < scene.getChild().size(); i++) {
            SceneObject shapeA = scene.getChild().get(i - 1);
            SceneObject shapeB = scene.getChild().get(i);
            if (shapeA instanceof Triangle a) {
                if(shapeB instanceof Triangle b) {
                ...
                if(shapeB instanceof Circle c) {
                    final Vector2D[] triangleWorld = a.toWorldCoordinate();
                    final Vector2D[] circleWorld = c.toWorldCoordinate();
                    final CollisionInfo collide = sat.collideTriangleCircle(triangleWorld, circleWorld);
                }
            }
